...
Authentication-only mode skips the admin/user role distinction for tokens validated by the configured issuer. Configured scope checks still apply. To use it, leave both `adminRole` and `userRole` empty and explicitly opt in:
...
Both `adminRole` and `userRole` must be set, or both must be empty. Setting only one is not supported. Helm rejects authentication-only mode unless `server.auth.allowOidcAuthOnly=true` is set. It also rejects this opt-in when OIDC is disabled or RBAC roles are configured.

<Warning>
  Before upgrading an existing OIDC deployment with both role values empty, configure both roles or set `server.auth.allowOidcAuthOnly=true`. Otherwise, Helm rejects the upgrade or the shared gateway fails to start.
</Warning>
