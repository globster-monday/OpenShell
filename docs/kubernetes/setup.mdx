...
When the proxy uses a private CA or intercepts upstream TLS, create a ConfigMap in each sandbox namespace with the PEM bundle under `ca-bundle.pem`:

```shell
kubectl -n openshell create configmap corporate-proxy-ca \
  --from-file=ca-bundle.pem=/path/to/corporate-ca-bundle.pem
```
...
upstreamProxy:
  ...
  caBundleConfigMapName: corporate-proxy-ca

supervisor:
  topology: sidecar
```

Use `authAllowInsecure: true` only when you accept that Basic authentication is cleartext on the connection to an `http://` proxy. HTTPS proxy endpoints do not require this acknowledgement because the supervisor verifies TLS before sending the credential. HTTP and HTTPS proxy endpoints support TLS CONNECT egress; forwarding plain HTTP egress remains unsupported.
