...
# sandbox workload cannot select or override these settings. HTTP and HTTPS proxy
...
# Optional ConfigMap in each sandbox namespace containing a PEM trust bundle
# under the ca-bundle.pem key. OpenShell mounts it read-only into the network
# supervisor and uses it for TLS to the proxy and intercepted upstream TLS.
