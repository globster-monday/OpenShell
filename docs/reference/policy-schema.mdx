...
| `on_error` | string | No | `fail_closed` denies the HTTP request or closes the WebSocket when the stage fails; `fail_open` skips a failed HTTP stage or disables a broken WebSocket stage for the rest of that connection. Defaults to `fail_closed`. |
