...
| `proxy_auth_allow_insecure` | `upstreamProxy.authAllowInsecure` | Set `true` to acknowledge that Basic authentication to an HTTP proxy is cleartext. Required with a proxy credential Secret only for `http://` proxy URLs. |
...
| `proxy_ca_bundle_config_map_name` | `upstreamProxy.caBundleConfigMapName` | Mount the `ca-bundle.pem` key from this ConfigMap into the network supervisor for private proxy and intercepted upstream TLS trust. |
